    ...
    , selectedClientId(-1)
{
    ...
    // dans le constructeur, section Employé connections :
    ...
    // Connect "🔍 Diagnostic" button if present in the UI
...
void MainWindow::onResetPasswordClicked()
{
    ...
    // ── Vérifier que le username correspond à un Responsable ──
    ...
    // ── Popup : demander l'ID pour vérifier l'identité ───────
    QDialog dlgId(this);
    ...
    connect(btnOk, &QPushButton::clicked, &dlgId, [&]() {
        ...
        // ── ID correct → mettre à jour le MDP ─────────────────
        QSqlQuery qUpdate;
        qUpdate.prepare(
    ...
    // ── Popup de confirmation avec les identifiants ───────────
    QDialog dlgOk(this);
...
bool MainWindow::validateLogin(const QString &username, const QString &password)
{
    ...
    // ── Connexion Responsable : username = resp_<nom> ─────────
    if (username.startsWith("resp_")) {
        // Extraire le nom depuis "resp_<nom>"
        QString nomRecherche = username.mid(5); // tout après "resp_"
...
{
    ui->inputIdEmploye->clear(); ui->inputNomEmploye->clear(); ui->inputPrenomEmploye->clear();
    ui->inputPosteEmploye->setCurrentIndex(0); ui->inputSalaireEmploye->setValue(0);
    ui->inputDateEmbaucheEmploye->setDate(QDate(2000,1,1));
}

// APRÈS (nouvelle version avec graphiques) :
void MainWindow::chargerStatistiquesEmployes()
{
    // ── Labels existants (inchangés) ──────────────────────────
    EmployeStats::charger(ui->valueTotalEmployes, ui->valueOuvriers, ui->valueTechniciens);

    // ── Graphiques dynamiques ─────────────────────────────────
    // Chercher si le QTabWidget de stats existe déjà
    QTabWidget *statTabs = ui->employeStatsTab->findChild<QTabWidget*>("statTabsEmploye");

    if (!statTabs) {
        // Créer le tabWidget la première fois seulement
        ...
        // Ajouter dans le layout existant de employeStatsTab
        QLayout *lay = ui->employeStatsTab->layout();
        if (lay)
            lay->addWidget(statTabs);
    }

    // Vider les anciens graphiques avant de recréer
...
void MainWindow::on_btnAddEmploye_clicked()
{
    ...
    // ── CAS RESPONSABLE : demander un mot de passe ────────────
    if (poste == "Responsable") {

        // ── Dialogue de saisie du MDP ─────────────────────────
        QDialog dlgMdp(this);
        ...
        // ── Construire le username : resp_<nom> ───────────────
        QString username = QString("resp_%1").arg(nom.toLower().remove(' '));

        // ── Popup de confirmation avec identifiants ───────────
        ...
        // ── Sauvegarder username dans la base ─────────────────
        // (optionnel : stocker le username dans une colonne ou table UTILISER)
        QSqlQuery qUser;
        qUser.prepare(
...
void MainWindow::on_btnTriEmploye_clicked()
{
    ...
    menu->setAttribute(Qt::WA_DeleteOnClose);  // auto-delete à la fermeture
    menu->setStyleSheet(
        "QMenu { background-color: white; border: 2px solid #5d732c; border-radius: 6px; padding: 4px; }"
        "QMenu::item { padding: 10px 20px; font-size: 13px; color: #2d4a1f; font-weight: bold; }"
        "QMenu::item:selected { background-color: #ffd700; color: #2d4a1f; border-radius: 4px; }"
    ...
    menu->addAction("Par nom (A-Z)", [this]() {
        afficherEmployesTable(EmployeTri::trierParNom(
            EmployeTri::chargerTousLesEmployes(), true));
    });
    menu->addAction("Par nom (Z-A)", [this]() {
        afficherEmployesTable(EmployeTri::trierParNom(
            EmployeTri::chargerTousLesEmployes(), false));
    });
    menu->addAction("Par prénom (A-Z)", [this]() {
        afficherEmployesTable(EmployeTri::trierParPrenom(
            EmployeTri::chargerTousLesEmployes(), true));
    });
    menu->addAction("Par poste", [this]() {
        afficherEmployesTable(EmployeTri::trierParPoste(
            EmployeTri::chargerTousLesEmployes()));
    });
    menu->addAction("Par salaire (↑ croissant)", [this]() {
        afficherEmployesTable(EmployeTri::trierParSalaire(
            EmployeTri::chargerTousLesEmployes(), true));
    });
    ...
    menu->addAction("Par date d'embauche (↑)", [this]() {
        afficherEmployesTable(EmployeTri::trierParDateEmbauche(
            EmployeTri::chargerTousLesEmployes(), true));
    });
    menu->addAction("Par date d'embauche (↓)", [this]() {
        afficherEmployesTable(EmployeTri::trierParDateEmbauche(
    ...
    // Affiche le menu juste sous le bouton Trier
...
void MainWindow::on_btnDiagnosticMachine_clicked()
{
    ...
    // Navigate to the digital twin panel
    ui->stackedWidget->setCurrentWidget(jumeau);

    // Sync machine list in case new machines were added
    jumeau->loadMachines();

    // Pre-select the machine that is currently highlighted in the table
