    ...
    , selectedClientId(-1)
{
    ...
    // Arduino: signaux de ArduinoZitTech
    connect(&arduinoZitTech, &ArduinoZitTech::idConfirmed,
            this, &MainWindow::onArduinoIdConfirmed);
    connect(&arduinoZitTech, &ArduinoZitTech::idInvalid,
            this, &MainWindow::onArduinoIdInvalid);
    connect(&arduinoZitTech, &ArduinoZitTech::serialError,
            this, &MainWindow::onArduinoSerialError);

    // Arduino: tentative de connexion au démarrage
    int arduinoRet = arduinoZitTech.connectArduino();
    switch (arduinoRet) {
    case 0:
        qDebug() << "[MainWindow] Arduino connected on port:"
                 << arduinoZitTech.getPortName();
        break;
    case 1:
        qDebug() << "[MainWindow] Arduino found but port open failed on:"
                 << arduinoZitTech.getPortName();
        break;
    case -1:
        qDebug() << "[MainWindow] Arduino not found. "
                    "Connect the Arduino Uno via USB to enable anomaly display.";
        break;
    }
    ...
    QPushButton *triEmployeBtn = ui->employeListTab->findChild<QPushButton*>("btnTriEmploye");
    if (triEmployeBtn)
        connect(triEmployeBtn, &QPushButton::clicked, this, &MainWindow::on_btnTriEmploye_clicked);
    ...
    // ── Arduino DHT11 (Machine — état thermique) ──────────────
    arduinoMachine = new ArduinoInterface(this);
    connect(arduinoMachine, &ArduinoInterface::donneesRecues,
            this, &MainWindow::onArduinoDonneesRecues);
    connect(arduinoMachine, &ArduinoInterface::ventilateurChange,
            this, &MainWindow::onArduinoVentilateurChange);
    connect(arduinoMachine, &ArduinoInterface::connexionChangee,
            this, &MainWindow::onArduinoConnexionChangee);
    connect(arduinoMachine, &ArduinoInterface::erreurSurvenue,
            this, &MainWindow::onArduinoErreurSurvenue);

    if (QPushButton *btnEtat = this->findChild<QPushButton*>("btnEtatMachine"))
        connect(btnEtat, &QPushButton::clicked,
                this, &MainWindow::on_btnEtatMachine_clicked);
...
void MainWindow::setDatabase(const QSqlDatabase &database)
{
    ...
    // Connexion automatique sur COM3
    if (rfidManager->connectToArduino("COM4")) {
        qDebug() << "✅ Arduino RFID connecté sur COM4";
    } else {
        qDebug() << "❌ Échec connexion COM4";
    }

    // ── Arduino DHT11 (Machine — état thermique) ──────────────
    if (arduinoMachine)
        arduinoMachine->setDatabase(database);
}
...
void MainWindow::chargerGraphiquesStatsStock()
{
    ...
    if (!statistiquesManager) return;

    // Le besoin UI: remplacer visuellement les blocs verts (statsRow1 + statsRow2)
    // par des graphiques, exactement dans la même zone.
    clearLayout(ui->statsRow1);
    clearLayout(ui->statsRow2);

    if (QChartView *v1 = statistiquesManager->creerGraphiqueRepartitionTypeStock())
    {
        v1->setMinimumSize(0, 0);
        v1->setMaximumHeight(260);
        v1->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
        ui->statsRow1->addWidget(v1, 1);
    }
    if (QChartView *v2 = statistiquesManager->creerGraphiqueQuantiteParOrigine())
    {
        v2->setMinimumSize(0, 0);
        v2->setMaximumHeight(260);
        v2->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
        ui->statsRow1->addWidget(v2, 1);
    }
    if (QChartView *v3 = statistiquesManager->creerGraphiqueNiveauxVsSeuils())
    {
        v3->setMinimumSize(0, 0);
        v3->setMaximumHeight(300);
        v3->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
        ui->statsRow2->addWidget(v3, 1);
    }

    // On masque la section graphique additionnelle en bas (devenue redondante).
    if (ui->statsChartsTitleStock)      ui->statsChartsTitleStock->setVisible(false);
    if (ui->frameStockStatsType)        ui->frameStockStatsType->setVisible(false);
    if (ui->frameStockStatsOrigine)     ui->frameStockStatsOrigine->setVisible(false);
    if (ui->frameStockStatsSeuils)      ui->frameStockStatsSeuils->setVisible(false);
}
...
void MainWindow::onAnalyseTerminee(const QList<ResultatDetection> &resultats)
{
    ...
    for (const ResultatDetection &r : resultats) {
        ...
        // Règle NOTIF :
        //   anomalie → "non"  orange gras  (pas encore notifié)
        //   normal   → "—"    style défaut (non applicable)
        //   "oui"            → uniquement après confirmation Arduino
        ...
        if (r.notifAffichee == "oui") {
            // Cas rare : déjà confirmé en BD depuis une analyse précédente
        ...
        // "—" → couleur par défaut, pas de gras
    ...
    // Envoyer les IDs des anomalies à l'Arduino pour confirmation keypad
    sendAnomalyIdsToArduino(resultats);
}
...
// ─────────────────────────────────────────────────────────────────────────────
//  ARDUINO — Slots et helpers
// ─────────────────────────────────────────────────────────────────────────────

// sendAnomalyIdsToArduino — Option B : relire la BD pour récupérer uniquement
// les productions avec NOTIF='non' (anomalies non encore notifiées).
// Le paramètre resultats n'est pas utilisé pour filtrer — la BD est la source
// de vérité, ce qui garantit la cohérence même si une confirmation a eu lieu
// entre deux analyses.
// Envoie "IDS:x,y,z\n" à l'Arduino pour mise à jour du LCD.
// Si aucune anomalie non notifiée, envoie "IDS:\n" (LCD → aucune anomalie).
void MainWindow::sendAnomalyIdsToArduino(const QList<ResultatDetection> &resultats)
{
    Q_UNUSED(resultats)   // On relit la BD directement (Option B)

    if (!arduinoZitTech.isConnected()) {
        qDebug() << "[MainWindow] Arduino not connected — skipping ID send.";
        return;
    }

    if (!QSqlDatabase::database().isOpen()) {
        qDebug() << "[MainWindow] sendAnomalyIdsToArduino: DB not open.";
        return;
    }

    // Lire depuis la BD les IDs dont NOTIF = 'non' (anomalie non notifiée)
    QList<int> nonNotifiedIds;
    QSqlQuery q;
    q.prepare(
        "SELECT ID_PROD FROM ZIT.PRODUCTION "
        "WHERE NOTIF = 'non' "
        "ORDER BY ID_PROD ASC");

    if (!q.exec()) {
        qDebug() << "[MainWindow] sendAnomalyIdsToArduino SQL error:"
                 << q.lastError().text();
        return;
    }

    while (q.next())
        nonNotifiedIds.append(q.value(0).toInt());

    if (nonNotifiedIds.isEmpty()) {
        qDebug() << "[MainWindow] No non-notified anomalies — sending CLEAR to Arduino.";
        arduinoZitTech.sendAnomalyIds({});
    } else {
        qDebug() << "[MainWindow] Sending" << nonNotifiedIds.size()
        << "non-notified IDs to Arduino (LCD):" << nonNotifiedIds;
        arduinoZitTech.sendAnomalyIds(nonNotifiedIds);
    }
}

// onArduinoIdConfirmed — appelé quand l'Arduino envoie "CONFIRM:<id>"
// L'ouvrier a tapé un ID valide sur le keypad.
// Actions :
//   1. UPDATE ZIT.PRODUCTION SET NOTIF='oui' WHERE ID_PROD=confirmedId
//   2. Mettre à jour la cellule "Notifier" dans anomalyTable → "oui" (vert)
//   3. Afficher une fenêtre de confirmation
void MainWindow::onArduinoIdConfirmed(int confirmedId)
{
    qDebug() << "[MainWindow] Arduino confirmed ID:" << confirmedId;

    if (updateNotifInDatabase(confirmedId)) {
        refreshNotifCellInTable(confirmedId);

        // Renvoyer la liste mise à jour à l'Arduino (Option B — relit la BD)
        // L'ID confirmé a maintenant NOTIF='oui' donc il sera exclu automatiquement
        sendAnomalyIdsToArduino({});

        QMessageBox::information(
            this,
            "✅ Notification confirmée",
            QString("La production n°%1 a été confirmée par l'ouvrier.\n\n"
                    "✔ Colonne 'Notifier' mise à jour : non → oui\n"
        ...
        QMessageBox::warning(
            ...
            QString("Impossible de confirmer l'ID %1.\n"
                    "Vérifiez que c'est une anomalie en attente (NOTIF='non').")
                .arg(confirmedId));
    }
}

// onArduinoIdInvalid — appelé quand l'Arduino envoie "INVALID:<id>"
void MainWindow::onArduinoIdInvalid(int typedId)
{
    qDebug() << "[MainWindow] Arduino: invalid ID:" << typedId;
    QMessageBox::warning(
        this,
        "⚠️ ID non valide",
        QString("L'ID %1 n'est pas une anomalie en attente.\n\n"
                "Il est soit normal, soit déjà confirmé.")
            .arg(typedId));
}

// onArduinoSerialError — appelé en cas d'erreur série (non-bloquant)
void MainWindow::onArduinoSerialError(const QString &message)
{
    qDebug() << "[MainWindow] Arduino serial error:" << message;
}

// updateNotifInDatabase — UPDATE ZIT.PRODUCTION SET NOTIF='oui'
//                         WHERE ID_PROD=productionId AND NOTIF='non'
// Retourne true si la mise à jour a réussi.
bool MainWindow::updateNotifInDatabase(int productionId)
{
    if (!QSqlDatabase::database().isOpen()) {
        qDebug() << "[MainWindow] DB not open in updateNotifInDatabase";
        return false;
    }

    // Vérifier que la ligne existe dans la table et est bien une anomalie en attente
    int targetRow = -1;
    for (int row = 0; row < ui->anomalyTable->rowCount(); ++row) {
        QTableWidgetItem *idItem = ui->anomalyTable->item(row, 0);
        if (!idItem) continue;
        if (idItem->text().toInt() == productionId) {
            targetRow = row;
            break;
        }
    }
    ...
    if (targetRow == -1) {
        qDebug() << "[MainWindow] updateNotifInDatabase: ID not found in anomalyTable =" << productionId;
        return false;
    }

    QTableWidgetItem *statutItem = ui->anomalyTable->item(targetRow, 4);
    QTableWidgetItem *notifItem  = ui->anomalyTable->item(targetRow, 5);

    if (!statutItem || !notifItem) {
        qDebug() << "[MainWindow] updateNotifInDatabase: missing cells for row" << targetRow;
        return false;
    }

    if (!statutItem->text().contains("Anomalie", Qt::CaseInsensitive)) {
        qDebug() << "[MainWindow] ID" << productionId << "is not anomalous.";
        return false;
    }

    if (notifItem->text().trimmed().toLower() != "non") {
        qDebug() << "[MainWindow] ID" << productionId << "not pending (NOTIF != non).";
        return false;
    }

    QSqlDatabase dbConn = QSqlDatabase::database();
    if (!dbConn.transaction()) {
        qDebug() << "[MainWindow] transaction begin failed:" << dbConn.lastError().text();
        return false;
    }
    ...
    query.prepare(
        ...
        "AND NOTIF = 'non'");
    query.bindValue(":id_prod", productionId);

    if (!query.exec()) {
        qDebug() << "[MainWindow] updateNotifInDatabase failed for ID"
                 << productionId << ":" << query.lastError().text();
        dbConn.rollback();
        return false;
    }

    if (query.numRowsAffected() == 0) {
        qDebug() << "[MainWindow] no row updated for ID" << productionId;
        dbConn.rollback();
        return false;
    }

    if (!dbConn.commit()) {
        qDebug() << "[MainWindow] commit failed:" << dbConn.lastError().text();
        dbConn.rollback();
        return false;
    }

    qDebug() << "[MainWindow] NOTIF='oui' updated for ID_PROD =" << productionId;
    return true;
}

// refreshNotifCellInTable — mettre à jour la cellule "Notifier" → "oui" (vert gras)
// pour la ligne correspondant à productionId dans anomalyTable.
void MainWindow::refreshNotifCellInTable(int productionId)
{
    for (int row = 0; row < ui->anomalyTable->rowCount(); ++row) {
        QTableWidgetItem *idItem = ui->anomalyTable->item(row, 0);
        if (!idItem) continue;
        ...
        if (idItem->text().toInt() == productionId) {
            QTableWidgetItem *notifItem = new QTableWidgetItem("oui");
            notifItem->setTextAlignment(Qt::AlignCenter);
            notifItem->setForeground(QColor("#28a745"));
            QFont f = notifItem->font();
            f.setBold(true);
            notifItem->setFont(f);
            notifItem->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEnabled);
            ui->anomalyTable->setItem(row, 5, notifItem);
            break;
        }
    }
...
// =============================================================================
//  ARDUINO DHT11 — État thermique Machine
// =============================================================================

void MainWindow::on_btnEtatMachine_clicked()
{
    if (!arduinoMachine) return;

    // Tentative de connexion si pas encore connecté
    if (!arduinoMachine->isConnected()) {
        if (!arduinoMachine->connectToArduino()) {
            return;
        }
    }

    // Construire le dialogue d'état thermique
    QDialog *dlg = new QDialog(this);
    dlg->setWindowTitle("🌡️ État Thermique de la Machine");
    dlg->setMinimumWidth(420);
    dlg->setAttribute(Qt::WA_DeleteOnClose);
    dlg->setWindowModality(Qt::NonModal);   // ← non-bloquant
    ...
    // ── Titre ────────────────────────────────────────────────
    QLabel *titre = new QLabel("🌡️ État Thermique (DHT11 — Arduino)");
    titre->setStyleSheet("font-size:16px; font-weight:bold; color:#2d4a1f;");
    titre->setAlignment(Qt::AlignCenter);
    vlay->addWidget(titre);

    QFrame *sep = new QFrame; sep->setFrameShape(QFrame::HLine);
    sep->setStyleSheet("color:#5d732c;"); vlay->addWidget(sep);

    // ── Helper pour créer une ligne label + valeur ─────────
    auto makeRow = [&](const QString &label, QLabel *&valLabel) {
        QHBoxLayout *h = new QHBoxLayout;
        QLabel *lbl = new QLabel(label);
        lbl->setStyleSheet("font-weight:bold; color:#2d4a1f; font-size:13px;");
        lbl->setMinimumWidth(160);
        valLabel = new QLabel("—");
        valLabel->setStyleSheet("font-size:13px; color:#333;");
        h->addWidget(lbl);
    ...
    QLabel *lblConnexion, *lblTemp, *lblHum, *lblSurchauffe, *lblVentil;
    makeRow("🔌 Connexion :",    lblConnexion);
    makeRow("🌡️ Température :", lblTemp);
    makeRow("💧 Humidité :",    lblHum);
    makeRow("🔥 Surchauffe :",  lblSurchauffe);
    makeRow("💨 Ventilateur :", lblVentil);

    // ── Bannière d'alerte surchauffe (cachée par défaut) ─────
    QLabel *lblAlerte = new QLabel("⚠️  SURCHAUFFE DÉTECTÉE — Ventilateur activé !");
    lblAlerte->setAlignment(Qt::AlignCenter);
    lblAlerte->setStyleSheet(
        "background:#dc3545; color:white; font-weight:bold; font-size:13px;"
        "padding:10px; border-radius:6px;");
    lblAlerte->setVisible(false);
    vlay->addWidget(lblAlerte);

    // ── Fonction refresh ─────────────────────────────────────
    auto refresh = [=]() {
        bool conn = arduinoMachine->isConnected();
        lblConnexion->setText(conn ? "✅ Connecté" : "❌ Déconnecté");
        lblTemp->setText(conn
                             ? QString("%1 °C").arg(arduinoMachine->getTemperature(), 0, 'f', 1)
                             : "—");
        lblHum->setText(conn
                            ? QString("%1 %").arg(arduinoMachine->getHumidite(), 0, 'f', 1)
                            : "—");
        bool surch = arduinoMachine->isSurchauffe();
        lblSurchauffe->setText(surch ? "⚠️ OUI" : "✅ NON");
        lblSurchauffe->setStyleSheet(surch
                                         ? "font-size:13px; color:#dc3545; font-weight:bold;"
                                         : "font-size:13px; color:#28a745;");
        lblVentil->setText(arduinoMachine->ventilateurActif() ? "🟢 ON" : "⚫ OFF");
        lblAlerte->setVisible(surch);   // bannière rouge visible seulement si surchauffe
    };

    // ── Timer polling 2s — garantit la mise à jour même si
    //    donneesRecues a été émis avant l'ouverture du dialogue ──
    QTimer *pollTimer = new QTimer(dlg);
    pollTimer->setInterval(2000);
    connect(pollTimer, &QTimer::timeout, dlg, [=]() { refresh(); });
    pollTimer->start();

    // ── Mise à jour immédiate sur chaque signal Arduino ───────
    connect(arduinoMachine, &ArduinoInterface::donneesRecues,
            dlg, [=](double, double, bool) { refresh(); });
    connect(arduinoMachine, &ArduinoInterface::ventilateurChange,
            dlg, [=](bool) { refresh(); });
    connect(arduinoMachine, &ArduinoInterface::connexionChangee,
            dlg, [=](bool) { refresh(); });
    ...
    // ── Boutons Ventilateur + Fermer ─────────────────────────
    QFrame *sep2 = new QFrame; sep2->setFrameShape(QFrame::HLine);
    sep2->setStyleSheet("color:#5d732c;"); vlay->addWidget(sep2);

    QHBoxLayout *btnLayout = new QHBoxLayout;
    QPushButton *btnOn    = new QPushButton("💨 Ventilateur ON");
    QPushButton *btnOff   = new QPushButton("⛔ Ventilateur OFF");
    QPushButton *btnClose = new QPushButton("Fermer");
    btnOn->setStyleSheet("background:#28a745; color:white; border:none; padding:10px 16px; border-radius:6px; font-weight:bold;");
    btnOff->setStyleSheet("background:#dc3545; color:white; border:none; padding:10px 16px; border-radius:6px; font-weight:bold;");
    btnClose->setStyleSheet("background:#ffd700; color:#2d4a1f; border:none; padding:10px 16px; border-radius:6px; font-weight:bold;");
    ...
    connect(btnOn,    &QPushButton::clicked, arduinoMachine, &ArduinoInterface::allumerVentilateur);
    connect(btnOff,   &QPushButton::clicked, arduinoMachine, &ArduinoInterface::eteindreVentilateur);
    connect(btnClose, &QPushButton::clicked, dlg, &QDialog::close);

    dlg->show();   // ← show() au lieu de exec() — ne bloque pas la boucle Qt
}

void MainWindow::onArduinoDonneesRecues(double temperature, double humidite, bool surchauffe)
{
    qDebug() << "[Machine/DHT11] Temp:" << temperature << "°C"
             << "| Hum:" << humidite << "%"
             << "| Surchauffe:" << (surchauffe ? "OUI" : "NON");
    // Les valeurs sont affichées en temps réel dans le dialogue État Machine.
    // Aucun popup — tout est visible directement dans le dialogue.
}

void MainWindow::onArduinoVentilateurChange(bool actif)
{
    qDebug() << "[Machine/DHT11] Ventilateur:" << (actif ? "ON" : "OFF");
}

void MainWindow::onArduinoConnexionChangee(bool connecte)
{
    qDebug() << "[Machine/DHT11] Connexion Arduino:" << (connecte ? "connecté" : "déconnecté");
}

void MainWindow::onArduinoErreurSurvenue(const QString &message)
{
    QMessageBox::warning(this, "❌ Erreur Arduino DHT11", message);
}
...
void MainWindow::onEmployeDetecte(const QString &nom,
                                  ...
                                  const QString &poste)
{
    qDebug() << "[RFID] Employé détecté :" << nom << prenom << "(" << poste << ")";
...
{
    qDebug() << "[RFID] Carte inconnue UID :" << uid;
