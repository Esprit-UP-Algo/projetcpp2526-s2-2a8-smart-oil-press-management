...
QVector<double> ClientPrevision::getHistoriqueQuantites(int idClient, const QString &produit)
{
    ...
    if (produit.isEmpty()) {
        query.prepare("SELECT QUANTITE FROM ZIT.ACHATS "
...
QVector<DonneeHistorique> ClientPrevision::getHistoriqueDetaille(int idClient, const QString &produit)
{
    ...
    if (produit.isEmpty()) {
        query.prepare("SELECT DATE_ACHAT, QUANTITE FROM ZIT.ACHATS "
                      "WHERE ID_CLIENT = :id ORDER BY DATE_ACHAT ASC");
        query.bindValue(":id", idClient);
    } else {
        query.prepare("SELECT DATE_ACHAT, QUANTITE FROM ZIT.ACHATS "
 ...
 */
double ClientPrevision::calculerTendanceRegression(const QVector<double> &donnees) const
{
    int n = donnees.size();
    if (n < 2) return 0.0;
 ...
 */
double ClientPrevision::calculerMoyenneMobilePondef(const QVector<double> &donnees, int periode) const
{
...
double ClientPrevision::predireQuantiteDepuisSerie(const QVector<double> &historique) const
{
    if (historique.isEmpty())
        return 0.0;

    const double mmp            = calculerMoyenneMobilePondef(historique, 4);
    const double tendance       = calculerTendanceRegression(historique);
    const double derniereVal    = historique.last();
    const double predRegression = qMax(0.0, derniereVal + tendance);

    const int n = historique.size();
    double wMMP = 0.4, wReg = 0.6;
    if (n < 4) {
        wMMP = 0.80;
        wReg = 0.20;
    } else if (n < 8) {
    ...
    return qMax(0.0, mmp * wMMP + predRegression * wReg);
}
...
QVector<PrevisionData> ClientPrevision::getPrevisionsTousClients()
{
    ...
    QSqlQuery query;
    query.exec("SELECT DISTINCT ID_CLIENT, PRODUIT FROM ZIT.ACHATS");
...
QVector<PrevisionData> ClientPrevision::getPrevisionsAgregeesParClient()
{
    const QVector<PrevisionData> detail = getPrevisionsTousClients();
    QMap<int, PrevisionData> parId;

    for (const PrevisionData &pv : detail) {
        if (pv.quantitePredite <= 0.0)
            continue;
        if (!parId.contains(pv.idClient)) {
            PrevisionData agg     = pv;
            agg.quantitePredite   = 0.0;
            agg.produit           = QStringLiteral("(tous produits)");
            agg.tendance          = 0.0;
            agg.saisonnalite      = 1.0;
            agg.confiance         = 0.0;
            agg.niveauConfiance   = QString();
            parId.insert(pv.idClient, agg);
        }
        PrevisionData &agg = parId[pv.idClient];
        agg.quantitePredite += pv.quantitePredite;
        agg.confiance = qMax(agg.confiance, pv.confiance);
        agg.tendance += pv.tendance;
    }

    QVector<PrevisionData> out;
    for (auto it = parId.constBegin(); it != parId.constEnd(); ++it) {
        PrevisionData d = it.value();
        d.niveauConfiance = niveauConfianceTexte(d.confiance);
        out.append(d);
    }
    std::sort(out.begin(), out.end(), [](const PrevisionData &a, const PrevisionData &b) {
        return a.quantitePredite > b.quantitePredite;
    });
    return out;
}
...
QChartView* ClientPrevision::creerGraphiquePrevisionsGlobal()
{
    ...
    QBarSet *set = new QBarSet(QStringLiteral("Quantité prévue (kg) — tous produits"));

    const QVector<PrevisionData> previsions = getPrevisionsAgregeesParClient();
    QStringList categories;
    int i = 0;
    const int n = qMax(1, previsions.size());

    for (const PrevisionData &pv : previsions) {
        if (pv.quantitePredite <= 0)
            continue;
        set->append(pv.quantitePredite);
        set->setColor(getCouleurVert(i, n));
        categories << pv.nomClient.trimmed();
        ++i;
    }
    ...
    chart->setTitle(QStringLiteral("Anticipation globale — demande cumulée par client"));
    chart->setAnimationOptions(QChart::SeriesAnimations);
    chart->setBackgroundBrush(QBrush(Qt::white));
    chart->setTitleFont(QFont("Arial", 13, QFont::Bold));
    chart->setTitleBrush(QBrush(QColor("#1b5e20")));
    ...
    axisX->setTitleText(QStringLiteral("Clients (total prévu sur l’horizon analysé)"));
    chart->addAxis(axisX, Qt::AlignBottom);
...
// ============================================================
//  GRAPHIQUE : DEUX CAMPAGNES — parts % par produit + besoin olives
// ============================================================

namespace {

/// Ordre de grandeur : kg d’olives au moulin pour 1 kg d’huile (réglable).
constexpr double KG_OLIVES_PAR_KG_HUILE = 5.2;

double previsionKgClient(int idc, const QString &campC, const QVector<Achat> &tous,
                         const ClientPrevision *prev)
{
    double            volC   = 0.0;
    int               nbCamp = 0;
    QVector<double>   serie;
    serie.reserve(tous.size());
    for (const Achat &a : tous) {
        if (a.idClient != idc)
            continue;
        serie.append(a.quantite);
        if (ClientHistorique::campagneOleicoleAnnee(a.dateAchat) == campC) {
            volC += a.quantite;
            ++nbCamp;
        }
    }
    if (volC > 0.0) {
        double ratio = 1.08;
        if (serie.size() >= 2) {
            const double predStep = prev->predireQuantiteDepuisSerie(serie);
            const double moyCamp  = volC / qMax(1, nbCamp);
            const double base     = qMax(1e-6, moyCamp);
            ratio                 = qBound(0.88, predStep / base, 1.35);
        }
        return qMin(volC * 2.8, volC * ratio);
    }
    if (!serie.isEmpty())
        return qMax(0.0, prev->predireQuantiteDepuisSerie(serie)
                             * qBound(3.0, static_cast<double>(serie.size()), 12.0));
    return 0.0;
}

} // namespace

QChartView* ClientPrevision::creerGraphiqueDeuxCampagnes()
{
    const QDate           today = QDate::currentDate();
    const QString         campC = ClientHistorique::campagneOleicoleAnnee(today);
    const QString         campS = ClientHistorique::campagneOleicoleAnneeSuivante(campC);
    const QVector<Achat>  tous = ClientHistorique::chargerTousLesAchats();

    QSet<int> ids;
    for (const Achat &a : tous)
        ids.insert(a.idClient);

    const QSqlDatabase db = QSqlDatabase::database();
    if (db.isOpen()) {
        QSqlQuery q(db);
        q.setForwardOnly(true);
        if (q.exec(QStringLiteral("SELECT ID_CLIENT FROM ZIT.CLIENT"))) {
            while (q.next())
    ...
    double totalPrevisionKg = 0.0;
    for (int idc : idList)
        totalPrevisionKg += previsionKgClient(idc, campC, tous, this);

    // Agrégation par produit — campagne courante (réalisé)
    QMap<QString, double> kgProduitCampC;
    for (const Achat &a : tous) {
        if (ClientHistorique::campagneOleicoleAnnee(a.dateAchat) != campC)
            continue;
        const QString cle = a.produit.trimmed().isEmpty() ? QStringLiteral("(sans nom)") : a.produit.trimmed();
        kgProduitCampC[cle] += a.quantite;
    }

    const double totalRealise = [&]() {
        double s = 0.0;
        for (auto it = kgProduitCampC.constBegin(); it != kgProduitCampC.constEnd(); ++it)
            s += it.value();
        return s;
    }();

    // Prévision kg par produit : même structure de mix que le réalisé, appliquée au tonnage prévu global
    QMap<QString, double> kgProduitPrev;
    if (totalRealise > 1e-9 && totalPrevisionKg > 1e-9) {
        for (auto it = kgProduitCampC.constBegin(); it != kgProduitCampC.constEnd(); ++it)
            kgProduitPrev[it.key()] = totalPrevisionKg * (it.value() / totalRealise);
    }
    ...
    for (auto it = kgProduitCampC.constBegin(); it != kgProduitCampC.constEnd(); ++it)
        cles.insert(it.key());
    for (auto it = kgProduitPrev.constBegin(); it != kgProduitPrev.constEnd(); ++it)
        cles.insert(it.key());

    QStringList categories;
    QList<double> pctReal;
    QList<double> pctPrev;

    auto pushRow = [&](const QString &nom, double kgr, double kgp) {
        categories << nom;
        const double pr = (totalRealise > 1e-9) ? (100.0 * kgr / totalRealise) : 0.0;
        const double pp = (totalPrevisionKg > 1e-9) ? (100.0 * kgp / totalPrevisionKg) : 0.0;
        pctReal.append(pr);
        pctPrev.append(pp);
    };

    const int maxBarres = 14;
    if (cles.isEmpty()) {
        pushRow(QStringLiteral("Aucune donnée"), 0.0, 0.0);
    } else {
        QList<QPair<QString, double>> tri;
        tri.reserve(cles.size());
        for (const QString &k : cles)
            tri.append({k, kgProduitCampC.value(k)});
        std::sort(tri.begin(), tri.end(), [](const QPair<QString, double> &a, const QPair<QString, double> &b) {
            return a.second > b.second;
        });

        double autreR = 0.0, autreP = 0.0;
        int    idx = 0;
        for (const auto &pr : tri) {
            const QString &nom = pr.first;
            const double   kr  = kgProduitCampC.value(nom);
            const double   kp  = kgProduitPrev.value(nom);
            if (++idx <= maxBarres)
                pushRow(nom, kr, kp);
            else {
                autreR += kr;
        ...
        if (autreR > 1e-9 || autreP > 1e-9)
            pushRow(QStringLiteral("Autres"), autreR, autreP);
    }

    auto *setReal = new QBarSet(campC + QStringLiteral("\n") + QStringLiteral("% ventes réalisées"));
    auto *setPrev = new QBarSet(campS + QStringLiteral("\n") + QStringLiteral("% prévision"));
    setReal->setBrush(QColor(QStringLiteral("#2e7d32")));
    setPrev->setBrush(QColor(QStringLiteral("#ef6c00")));
    for (int i = 0; i < categories.size(); ++i) {
        setReal->append(pctReal.at(i));
        setPrev->append(pctPrev.at(i));
    }

    auto *series = new QBarSeries();
    series->append(setReal);
    series->append(setPrev);

    auto *chart = new QChart();
    chart->addSeries(series);

    double pctHuile = 0.0, pctOlive = 0.0, pctAutre = 0.0;
    double kgHuilePrev = 0.0;
    if (totalPrevisionKg > 1e-9) {
        for (auto it = kgProduitPrev.constBegin(); it != kgProduitPrev.constEnd(); ++it) {
            const QString nk = it.key().toLower();
            const double frac = 100.0 * it.value() / totalPrevisionKg;
            if (nk.contains(QStringLiteral("huile"))) {
                pctHuile += frac;
                kgHuilePrev += it.value();
            } else if (nk.contains(QStringLiteral("olive"))) {
                pctOlive += frac;
            } else {
                pctAutre += frac;
            }
    ...
    const double tonnesOlivesPressoir = kgHuilePrev * KG_OLIVES_PAR_KG_HUILE / 1000.0;

    const QString ligneDetail = QStringLiteral(
                                    "Les barres représentent la part de chaque référence dans le volume total "
                                    "(≈100 %% par campagne). "
                                    "Pour %1 — prévision : ≈ %2 %% huile, ≈ %3 %% olives / dérivés, ≈ %4 %% autres. "
                                    "Besoin pressoir (olives) pour couvrir la part huile : ≈ %5 t d’olives "
                                    "(règle %6 kg olives / kg huile).")
                                    .arg(campS,
                                         QString::number(pctHuile, 'f', 1),
                                         QString::number(pctOlive, 'f', 1),
                                         QString::number(pctAutre, 'f', 1),
                                         QString::number(tonnesOlivesPressoir, 'f', 2),
                                         QString::number(KG_OLIVES_PAR_KG_HUILE, 'f', 1));

    chart->setTitle(QStringLiteral("Répartition par produit — parts du volume (%%) — %1 vs %2\n%3")
                        .arg(campC, campS, ligneDetail));

    chart->setAnimationOptions(QChart::SeriesAnimations);
    chart->setBackgroundBrush(QBrush(Qt::white));
    chart->setTitleFont(QFont(QStringLiteral("Arial"), 12, QFont::Bold));
    chart->setTitleBrush(QBrush(QColor(QStringLiteral("#1b5e20"))));

    auto *axisX = new QBarCategoryAxis();
    axisX->append(categories);
    axisX->setTitleText(QStringLiteral("Produits"));
    chart->addAxis(axisX, Qt::AlignBottom);
    series->attachAxis(axisX);

    auto *axisY = new QValueAxis();
    axisY->setTitleText(QStringLiteral("Part du volume total (%)"));
...
void ClientPrevision::exporterHistoriquePDF(int idClient, const QString &cheminFichier, QWidget *parentWidget)
{
    ...
    QPainter p;
    if (!p.begin(&printer)) {
        QMessageBox::critical(parentWidget, "Erreur PDF", "Impossible de créer le fichier PDF.");
        return;
    }
    ...
    p.end();
    QMessageBox::information(parentWidget, "Export PDF",
