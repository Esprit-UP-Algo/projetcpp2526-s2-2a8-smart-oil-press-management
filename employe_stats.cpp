...
{
    QSqlQuery query;
    query.exec("SELECT NOM, PRENOM, "
               "TRUNC(MONTHS_BETWEEN(SYSDATE, DATE_EMBAUCHE) / 12) AS ANNEES "
               "FROM EMPLOYE ORDER BY DATE_EMBAUCHE ASC");

    QStringList noms;
    QList<int> anneesList;

    while (query.next()) {
        noms       << (query.value(0).toString() + " " + query.value(1).toString());
        anneesList << query.value(2).toInt();
    }
    ...
    // Un QBarSet par employé avec 0 partout sauf à sa position
    // → une barre colorée unique par ligne
    for (int i = 0; i < noms.size(); i++) {
        QBarSet *set = new QBarSet("");
        for (int j = 0; j < noms.size(); j++)
            *set << (j == i ? anneesList[i] : 0);
        set->setColor(getCouleurDegradeVert(i, qMax(noms.size(), 2)));
        set->setBorderColor(QColor("#1b5e20"));
        set->setLabelFont(QFont("Arial", 9, QFont::Bold));
        series->append(set);
    }
