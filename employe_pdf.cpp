...
void EmployePDF::exporter(QTableWidget *table, QWidget *parent)
{
    // ── 1. Choix du fichier de destination ───────────────────
    QString defaultName = QDir::homePath() + "/Employes_"
    ...
    // ── 2. Configuration imprimante PDF ──────────────────────
    ...
    QStringList colHeaders = {"ID", "Nom", "Prénom", "Poste", "Salaire (DT)", "Date d'Embauche"};
    int nCols = colHeaders.size();

    QVector<double> colRatio = {0.06, 0.18, 0.18, 0.18, 0.16, 0.18};
    ...
    auto drawHeader = [&](int page) {
        ...
        p.drawText(QRect(tX, (int)(headerH*0.55), W-tX-margin, (int)(headerH*0.38)),
                   Qt::AlignVCenter | Qt::AlignLeft, "Liste des Employés");
        ...
        p.drawText(QRect(mX,(int)(headerH*0.52),mW,(int)(headerH*0.38)),
                   Qt::AlignVCenter|Qt::AlignRight,
                   QString("Employés : %1   |   Page %2 / %3").arg(nbRows).arg(page).arg(totalPages));
    ...
    auto drawRow = [&](int rowIdx, int y, bool even) {
        ...
        for (int c = 0; c < nCols && c < table->columnCount(); c++) {
            ...
            if (c == 4 && !txt.isEmpty()) txt += " DT";   // Colonne salaire

            // Poste en vert gras, reste en noir
            if (c == 3) { p.setPen(C_MID_GREEN); p.setFont(font("Arial",9,true)); }
    ...
    while (true) {
        ...
        // Lignes vides pour fermer proprement le tableau
