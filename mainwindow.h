...
// ── RFID / Arduino Uno R4 (module Employé) ───────────────────
...
// ── Arduino Uno (Production module — anomaly notification) ────
#include "arduino_zittech.h"

// ── Arduino DHT11 (Machine module — état thermique) ──────────
#include "arduino.h"
...
private slots:
    ...
    // ── Production Arduino (Uno — anomaly notification) ───────
    ...
    void on_btnEtatMachine_clicked();
    void onArduinoDonneesRecues(double temperature, double humidite, bool surchauffe);
    void onArduinoVentilateurChange(bool actif);
    void onArduinoConnexionChangee(bool connecte);
    void onArduinoErreurSurvenue(const QString &message);

    // ── RFID (module Employé — Arduino Uno R4) ────────────────
    void onEmployeDetecte(const QString &nom, const QString &prenom, const QString &poste);
    void onCarteInconnue(const QString &uid);

private:
    Ui::MainWindow *ui;
    QSqlDatabase    db;

    // ── RFID manager (module Employé) ─────────────────────────
    ...
    void chargerGraphiquesStatsStock();
    ...
    // ── Arduino DHT11 (Machine — état thermique) ──────────────
    ArduinoInterface *arduinoMachine = nullptr;
};
