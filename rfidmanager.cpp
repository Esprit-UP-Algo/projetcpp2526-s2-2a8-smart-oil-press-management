...
void RfidManager::processUID(const QString &uid)
{
    ...
    if (!db.isValid() || !db.isOpen()) {
        qDebug() << "[RFID] processUID: DB invalide ou fermée pour UID:" << uid;
    ...
    if (!query.exec()) {
        qDebug() << "[RFID] Erreur SQL:" << query.lastError().text();
        sendToArduino("DB_ERROR");
        return;
    }
